...
mov EAX,65536	;В регистре EAX задано число 65536 (можно от 0 до 4294967295)
mov EBX,10	;аносим основание системы счисления, равное 10, в регистр BX

mov CX,0	;1. Номер младшего разряда числа, равный 0, заносим в регистр CX.

div_num:
    mov EDX,0	;2. Заносим 0 в регистр DX (после деления в регистр DX попадет остаток от деления, перед делением надо его обнулить, т.к. делимое у нас DX:AX). Также DX содержит старшую значимую часть делимого(у нас всегда 0)
    div EBX		;3. Выполняем деление DX:AX на BX: неполное частное от деления помещается в AX (div), а остаток (mod) - в DX.
    push EDX		;4. Сохраняем содержимое DX в стеке (нам надо сохранить очередную цифру в стек)
    inc CX	    ;5. Увеличиваем значение счетчика в регистре CX на 1.(инкремент)
    cmp EAX,0	;6.Проверка условия: если неполное частное равно 0, то переход на пункт 7, иначе переход на пункт 2.
    jne div_num	;переход к метке div_num, если метка ZF = 0(числа не равны)

loop_:		;7. Организуем цикл loop, в котором:
    pop EDX      ;a. извлекаем из стека слово данных в регистр DX.
